...
```{r}
sprintf("%s ate %d cookies.", 'John', 24)
```

```{r}
sprintf("%f %e", 21.34,21.34)
```
...
```{r}
# 2What values can a BOOLEAN contain?  TRUE or FALSE
```
...
1.  Create two strings with your first and last name, respectively. Then concatenate them, and print the new value as "My name is \<your newly created concatenated variable will go here\>"
    ...
    first_name <- 'Sean'
    last_name <- 'Bunk'
    fullName <- paste(first_name,last_name, se = " ")
    fullName
    cat("My name is",fullName)
    ```
